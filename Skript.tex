...
\begin{document}
    ...
    \begin{definition}[Zerlegung eines Intervalls]
        ...
        \begin{enumerate}[label=(\alph*)]
            \item Eine Zerlegung $Z$ eines kompakten Intervalls $I=\interv{a,b}$ in Teilintervalle $I_j$ der Längen $\abs{I_j}$ ($j=1,\dots, k$) ist eine Menge von Punkten $x_0, x_1,\dots, x_k\in I$ (Teilpunkte von $Z$) mit $a=x_0 < x_1 < x_2 < \dots < x_k = b$.
            \item Wir setzen $\varDelta x_j \definedas x_j - x_{j-1} \definedasbackwards\abs{I_j}$
            \item Die Feinheit der Zerlegung $Z$ ist $\varDelta\of{z}\definedas \max\of{\abs{I_1}, \abs{I_2}, \dots, \abs{I_k}} = \max\of{\varDelta x_1, \varDelta x_2, \dots, \varDelta x_k}$ (Länge des längsten Teilintervalls von $Z$).
        \end{enumerate}
        An jedem $I_j$ wählen wir $\xi_j\in I_j$, setzen $\xi=\pair{\xi_1, \xi_2, \dots, \xi_k}$, setzen $B\of{I} =$ Menge aller beschränkten reelwertigen Funktionen auf $I$ $= \set{f: I\fromto \R: \sup_{x\in I} \abs{f(x)} < \infty}$.\\
        Für $f\in B\of{I}$ setzen
        \begin{align*}
            S_z\of{f} &= S_z\of{f, \xi} \definedas \sum_{j=1}^{k} f\of{\xi_j}\varDelta x_j = \sum_{j=1}^{k} f\of{\xi_j}\abs{I_j}\tag{Riemannsche Zwischensumme}
        \end{align*}
        Für $f\in B\of{I}$ setzen
        \begin{align*}
            \underline{m}_j &\definedas \inf_{I_j} f = \inf\set{f(x): x\in I_j}\\
            \overline{m}_j &\definedas \sup_{I_j} f = \sup\set{f(x): x\in I_j}\\
            \overline{S}_z\of{f}&\definedas \sum_{j=1}^{k} \overline{m}_j \varDelta x_j\tag{Obersumme}\\
            \underline{S}_z\of{f}&\definedas \sum_{j=1}^{k} \underline{m}_j \varDelta x_j\tag{Untersumme}
            \intertext{Haben}
            \underline{m}_j &\leq f\of{x} \leq \overline{m}_j \text{ für } x \in I_j\\
